...
namespace
{
	...
	double NormalizeBarConfigZoom(double configZoom)
	{
		if (!isfinite(configZoom)) return 1.0;
		return clamp(configZoom, BarConfigZoomMin, BarConfigZoomMax);
	}

	double RoundBarConfigZoom(double configZoom)
	{
		return round(NormalizeBarConfigZoom(configZoom) * 100.0) / 100.0;
	}

	double FloorBarConfigZoom(double configZoom)
	{
		if (!isfinite(configZoom)) return 1.0;
	...
	void RefreshBarZoom(BarUISetClass& barUISet)
	{
	...
	void ApplyBarConfigZoom(BarUISetClass& barUISet, double configZoom, bool keepCurrentScreenPosition)
	{
		configZoom = RoundBarConfigZoom(configZoom);
	...
	void FitInitialZoomAfterMainBarLayout(BarUISetClass& barUISet, double mainBarWidth)
	{
		...
		double fitConfigZoom = FloorBarConfigZoom(fitZoom / dpiZoom);
		if (fitConfigZoom >= barUISet.barStyle.configZoom.load()) return;

		Inkeys::config.UI.Bar.Zoom = fitConfigZoom;
		ApplyBarConfigZoom(barUISet, fitConfigZoom, false);
		Inkeys::config.Write();
	}
}
...
void BarUISetClass::Rendering()
{
	...
	#pragma region 计算UI
		...
		// 主栏
		{
			...
			FitInitialZoomAfterMainBarLayout(*this, totalWidth);
			{ /**/ }
...
namespace Inkeys::UI::Bar
{
	void SetConfigZoom(double configZoom)
	{
		...
		ApplyBarConfigZoom(barUISet, configZoom, true);
	}
	...
	{
		// UI DPI 缩放（启动时系统 DPI * 配置倍率）
		{
			double configZoom = RoundBarConfigZoom(Inkeys::config.UI.Bar.Zoom.load());
