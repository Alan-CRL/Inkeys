...
ImGuiToggleConfig config{
	...
	.Flags = ImGuiToggleFlags_Animated | ImGuiToggleFlags_ShadowedFrame
};
...
void SettingStartUp()
{
#pragma region 图像媒体加载

#pragma endregion

	// 图像加载 TODO 需要直接用 imstb 加载进去，从资源加载
	{
		// Home1
		{
			constexpr auto LoadImg = ImgEnum::Home1;

			void* imgData = nullptr;
			DWORD imgSize = 0;
			if (I18n::identifying == L"zh-CN") Inkeys::Load::ExtractResourcePtr(imgData, imgSize, L"PNG", L"Home1_zh-CN");
			else if (I18n::identifying == L"zh-TW") Inkeys::Load::ExtractResourcePtr(imgData, imgSize, L"PNG", L"Home1_zh-TW");
			else Inkeys::Load::ExtractResourcePtr(imgData, imgSize, L"PNG", L"Home1_en-US");

			Img[LoadImg].width = static_cast<int>(700 * settingGlobalScale);
			Img[LoadImg].height = static_cast<int>(215 * settingGlobalScale);
			bool ret = LoadTextureFromMemory(imgData, imgSize, Img[LoadImg].img.ReleaseAndGetAddressOf(), Img[LoadImg].width, Img[LoadImg].height);
		}
		// Home2
		{
			constexpr auto LoadImg = ImgEnum::Home2;

			void* imgData = nullptr;
			DWORD imgSize = 0;
			if (I18n::identifying == L"zh-CN") Inkeys::Load::ExtractResourcePtr(imgData, imgSize, L"PNG", L"Home2_zh-CN");
			else if (I18n::identifying == L"zh-TW") Inkeys::Load::ExtractResourcePtr(imgData, imgSize, L"PNG", L"Home2_zh-TW");
			else Inkeys::Load::ExtractResourcePtr(imgData, imgSize, L"PNG", L"Home2_en-US");

			Img[LoadImg].width = static_cast<int>(770 * settingGlobalScale);
			Img[LoadImg].height = static_cast<int>(390 * settingGlobalScale);
			bool ret = LoadTextureFromMemory(imgData, imgSize, Img[LoadImg].img.ReleaseAndGetAddressOf(), Img[LoadImg].width, Img[LoadImg].height);
		}
		// PluginFlag1
		{
			constexpr auto LoadImg = ImgEnum::PluginFlag1;

			void* imgData = nullptr;
			DWORD imgSize = 0;
			Inkeys::Load::ExtractResourcePtr(imgData, imgSize, L"PNG", L"PluginFlag1");

			Img[LoadImg].width = static_cast<int>(30 * settingGlobalScale);
			Img[LoadImg].height = static_cast<int>(30 * settingGlobalScale);
			bool ret = LoadTextureFromMemory(imgData, imgSize, Img[LoadImg].img.ReleaseAndGetAddressOf(), Img[LoadImg].width, Img[LoadImg].height);
		}
...

			{
				...
				// 左侧导航栏
				{
					...
					// 实验选项
					{
						...
						if (settingTab == settingTabEnum::tabExperimental)
						{
							PushStyleColorNum++, ImGui::PushStyleColor(ImGuiCol_Button, IM_COL32(0, 0, 0, 10));
							PushStyleColorNum++, ImGui::PushStyleColor(ImGuiCol_ButtonHovered, IM_COL32(0, 0, 0, 10));
							PushStyleColorNum++, ImGui::PushStyleColor(ImGuiCol_ButtonActive, IM_COL32(0, 0, 0, 10));
						...
						PushStyleVarNum++, ImGui::PushStyleVar(ImGuiStyleVar_ButtonTextAlign, ImVec2(0.0f, 0.5f));
						if (ImGui::Button(("   \uec4a   " + string("实验选项")).c_str(), { 150.0f * settingGlobalScale,36.0f * settingGlobalScale })) settingTab = settingTabEnum::tabExperimental;
					}
