...
int pptTotalSlides = -1, pptCurrentSlides = -1;
...
void PptDraw()
{
	...
	{
		pptTotalSlides = PptInfoState.TotalPage;
		pptCurrentSlides = PptInfoState.CurrentPage;
		...
		// 控件加载部分
		if (!Initialization && pptTotalSlides != -1)
		{
			ppt_show = GetPptShow();
		...
		else if (Initialization && pptTotalSlides == -1)
		{
			PptImg.IsSave = false;
