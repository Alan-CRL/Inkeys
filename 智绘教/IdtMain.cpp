...
string edition_date = "20231106a"; //程序发布日期
string edition_code = "2311 - BUG Bash"; //程序版本
...
int main()
{
	...
	for (; i <= 10; i++)
	{
		if (!thread_status[L"floating_main"] && !thread_status[L"drawpad_main"] && !thread_status[L"test_main"] && !thread_status[L"NetUpdate"]) break;
		Sleep(500);
	}
