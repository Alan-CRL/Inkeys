// Inkeys 开始魔改（基于当前文件 29269119 版；IMGUI 1.92.6）
import Inkeys.Load;
...
bool    ImGui_ImplDX11_CreateDeviceObjects()
{
	...
	{
		// [Inkeys] 我们对以下部分进行了修改：
		// 加载内存资源中的 cso 文件，而不是动态编译
		// 这使得二进制文件在目标计算机上不再依赖 d3dcompiler_47.dll
		// 同时，目标纹理为 BGRA 格式，我们使用像素着色器将 RGBA 转换为 BGRA
		...
		// IDR_SHADERS1 and IDR_SHADERS2
		if (!Inkeys::Load::ExtractResourcePtr(vsData, vsSize, L"SHADERS", MAKEINTRESOURCEW(299)) ||
			!Inkeys::Load::ExtractResourcePtr(psData, psSize, L"SHADERS", MAKEINTRESOURCEW(300)))
		{
			return false;
		}

		if (bd->pd3dDevice->CreateVertexShader(vsData, vsSize, nullptr, &bd->pVertexShader) != S_OK)
		{
			return false;
		}
		if (bd->pd3dDevice->CreatePixelShader(psData, psSize, nullptr, &bd->pPixelShader) != S_OK)
		{
		...
		if (bd->pd3dDevice->CreateInputLayout(local_layout, 3, vsData, vsSize, &bd->pInputLayout) != S_OK)
		{
			if (bd->pVertexShader) { bd->pVertexShader->Release(); bd->pVertexShader = nullptr; }
		...
		{
			D3D11_BUFFER_DESC desc = {};
			desc.ByteWidth = sizeof(VERTEX_CONSTANT_BUFFER_DX11); // 通常为 64 字节
			desc.Usage = D3D11_USAGE_DYNAMIC;
			desc.BindFlags = D3D11_BIND_CONSTANT_BUFFER;
			desc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
			desc.MiscFlags = 0;
			if (bd->pd3dDevice->CreateBuffer(&desc, nullptr, &bd->pVertexConstantBuffer) != S_OK)
			{
				if (bd->pVertexShader) { bd->pVertexShader->Release(); bd->pVertexShader = nullptr; }
				if (bd->pPixelShader) { bd->pPixelShader->Release(); bd->pPixelShader = nullptr; }
