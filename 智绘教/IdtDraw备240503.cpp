...
double pointToLineDistance(PointF lineStart, PointF lineEnd, PointF p) {
...
double pointToLineSegmentDistance(PointF lineStart, PointF lineEnd, PointF p)
{
...
bool isLine(vector<PointF> points, int tolerance, std::chrono::high_resolution_clock::time_point start)
{
	int n = points.size();
	if (n < 2) return false;
	if (n == 2) return true;
