...
void MultiFingerDrawing(LONG pid, POINT pt)
{
	...
	else if (draw_info.brush_choose == true)
	{
		...
		vector<PointF> ActualPoints = { PointF(mouse.last_x, mouse.last_y) };
		vector<PointF> SmoothPoints;
		vector<Result> IdtSmoothedStroke;

		// 书写模式试用 实时平滑笔迹
		StrokeModeler modeler;
		if (draw_info.mode == 1)
		{
			Duration IdtParamsTimeOut(15);
			StrokeModelParams IdtParams
			{
				.wobble_smoother_params
				{
					.timeout = IdtParamsTimeOut,
					.speed_floor = 0.01,
					.speed_ceiling = 0.03
				},
			...
			modeler.Reset(IdtParams);

			{
				Time IdtParamsTime(chrono::duration_cast<chrono::milliseconds>(chrono::system_clock::now().time_since_epoch()).count());
				Input IdtPointerInput
				{
					.event_type = Input::EventType::kDown,
					.position{(float)pt.x, (float)pt.y},  // 某些一致单位中的 x, y 坐标
					.time = IdtParamsTime,  // 某些一致单位中的时间
					// 以下字段是可选的：
					//.pressure = .2,  // 压力从 0 到 1
					//.orientation = M_PI  // 屏幕平面内的角度，以弧度为单位
					//.tilt = 0,  // 从屏幕平面抬高的角度，以弧度为单位
				};

				modeler.Update(IdtPointerInput, IdtSmoothedStroke);
			}
		}
		...
		while (1)
		{
			...
			// 延迟拉直
			if (draw_info.mode == 1 && !StopTimingDisable && !ActualPoints.empty())
			{
				...
				else if (chrono::duration_cast<std::chrono::milliseconds>(std::chrono::high_resolution_clock::now() - StopTiming).count() >= 1000)
				{
					if (sqrt((pt.x - ActualPoints[0].X) * (pt.x - ActualPoints[0].X) + (pt.y - ActualPoints[0].Y) * (pt.y - ActualPoints[0].Y)) >= 120)
					{
						...
						// 5 倍宽松精度
						if (isLine(ActualPoints, int(redundance * 5.0f), std::chrono::high_resolution_clock::now()))
						{
							draw_info.mode = 3;
							mouse.last_x = ActualPoints[0].X, mouse.last_y = ActualPoints[0].Y;
						}
			...
			//if (EuclideanDistance(PointF(mouse.last_x, mouse.last_y), PointF(pt.x, pt.y)) <= 1) continue;
			...
			else if (draw_info.mode == 4)
			{
				...
				if (ActualPoints.size() < 2) ActualPoints.push_back(PointF(pt.x, pt.y));
				else ActualPoints[1] = PointF(pt.x, pt.y);
			}
			else
			{
				// 点集处理
				{
					writing_distance += EuclideanDistance(PointF(mouse.last_x, mouse.last_y), PointF(pt.x, pt.y));
					ActualPoints.emplace_back(PointF(pt.x, pt.y));
		...
		if (draw_info.mode == 1)
		{
			//直线绘制
			double redundance = max(GetSystemMetrics(SM_CXSCREEN) / 192, min((GetSystemMetrics(SM_CXSCREEN)) / 76.8, double(GetSystemMetrics(SM_CXSCREEN)) / double((-0.036) * writing_distance + 135)));
			if (setlist.IntelligentDrawing && writing_distance >= 120 && (abs(circumscribed_rectangle.left - circumscribed_rectangle.right) >= 120 || abs(circumscribed_rectangle.top - circumscribed_rectangle.bottom) >= 120) && isLine(ActualPoints, int(redundance), std::chrono::high_resolution_clock::now()))
			{
				PointF start(ActualPoints[0]), end(ActualPoints[ActualPoints.size() - 1]);

				//端点匹配
				{
					//起点匹配
					{
						PointF start_target = start;
						double distance = 10;
						...
						for (const auto& [point, value] : extreme_point)
						{
							if (value == true)
							{
								if (EuclideanDistance(PointF(point.first, point.second), PointF(start.X, start.Y)) <= distance)
								{
									distance = EuclideanDistance(PointF(point.first, point.second), PointF(start.X, start.Y));
									start_target = PointF(point.first, point.second);
								}
							}
						}
						LockExtremePointSm.unlock();
					...
					//终点匹配
					{
						...
						for (const auto& [point, value] : extreme_point)
						{
							if (value == true)
							{
								if (EuclideanDistance(PointF(point.first, point.second), PointF(end.X, end.Y)) <= distance)
								{
									distance = EuclideanDistance(PointF(point.first, point.second), PointF(end.X, end.Y));
									end_target = PointF(point.first, point.second);
								}
							}
						}
						LockExtremePointSm.unlock();
			...
			else if (setlist.SmoothWriting && ActualPoints.size() > 2)
			{
				// 平滑准备
				{
					...
					//Testi(ActualPoints.size());

					for (long long i = 0; i < (long long)IdtSmoothedStroke.size(); i++)
					{
						SmoothPoints.emplace_back(PointF(IdtSmoothedStroke[i].position.x, IdtSmoothedStroke[i].position.y));

						//cout << i << " " << IdtSmoothedStroke[i].velocity.x << "," << IdtSmoothedStroke[i].velocity.y << endl;
					}
					Testi(IdtSmoothedStroke.size());
				}
				...
				graphics.DrawLines(&pen, SmoothPoints.data(), SmoothPoints.size());
			}
		}
		...
		{
			//端点匹配
			if (setlist.IntelligentDrawing && ActualPoints.size() == 2)
			{
				PointF l1 = ActualPoints[0];
				PointF l2 = PointF(ActualPoints[0].X, ActualPoints[ActualPoints.size() - 1].Y);
				PointF r1 = PointF(ActualPoints[ActualPoints.size() - 1].X, ActualPoints[0].Y);
				PointF r2 = ActualPoints[ActualPoints.size() - 1];
				...
				{
					{
						...
						for (const auto& [point, value] : extreme_point)
						{
							if (value == true)
							{
								if (EuclideanDistance(PointF(point.first, point.second), PointF(l1.X, l1.Y)) <= distance)
								{
									distance = EuclideanDistance(PointF(point.first, point.second), PointF(l1.X, l1.Y));
									idx = PointF(point.first, point.second);
								}
							}
						}
						LockExtremePointSm.unlock();
					...
					}
					{
						...
						for (const auto& [point, value] : extreme_point)
						{
							if (value == true)
							{
								if (EuclideanDistance(PointF(point.first, point.second), PointF(l2.X, l2.Y)) <= distance)
								{
									distance = EuclideanDistance(PointF(point.first, point.second), PointF(l2.X, l2.Y));
									idx = PointF(point.first, point.second);
								}
							}
						}
					...
					}
					{
						...
						for (const auto& [point, value] : extreme_point)
						{
							if (value == true)
							{
								if (EuclideanDistance(PointF(point.first, point.second), PointF(r1.X, r1.Y)) <= distance)
								{
									distance = EuclideanDistance(PointF(point.first, point.second), PointF(r1.X, r1.Y));
									idx = PointF(point.first, point.second);
								}
							}
						}
					...
					}
					{
						...
						for (const auto& [point, value] : extreme_point)
						{
							if (value == true)
							{
								if (EuclideanDistance(PointF(point.first, point.second), PointF(r2.X, r2.Y)) <= distance)
								{
									distance = EuclideanDistance(PointF(point.first, point.second), PointF(r2.X, r2.Y));
									idx = PointF(point.first, point.second);
								}
							}
						}
