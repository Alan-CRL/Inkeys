...
- 跨 Inkeys 进程恢复是未来目标，但必须在页面内容冲突、整页插入/删除/重排、身份变化及用户提示策略完成后再开放。
...
- PowerPoint 与 WPS 官方对象模型都提供 SlideID；PptCOM 仍可能因 COM 接口损坏而退回纯 dynamic 访问并无法取得 ID。本任务允许此时按页序号退化保存，并只在同进程、同 PPT binding、相同页序号条件下恢复，不承诺插删页后的稳定绑定。
...
- R4. Presentation 画布按 PPT 文档和页码组织。同一 PPT 的所有页映射到同一个 UInk Document 的 Canvas 集合，不同 PPT 不得互相覆盖。
...
- R8. 在当前 Inkeys 进程内，对于已经由本进程自动保存并登记索引的 PPT，再次进入同一放映时加载整个 UInk Canvas 集合，并按当前页切换到对应 Canvas。
...
- R13. PptCOM 应优先取得真实 SlideID；ID 不可得时使用显式 `PageIndexFallback` 绑定模式，以页序号保存 Canvas。稳定路径文稿仅允许当前进程同 session/key/source、相同页数按 ordinal 恢复，process-local 身份仍限 exact binding；退化记录不得被未来跨进程恢复误认为稳定 SlideID 记录。
...
- [x] AC6. 当前进程内退出并再次进入同一 PPT 放映，会从索引命中的 `.uink` 恢复全部已保存页，当前页显示对应 Canvas；进程重启后的自动恢复不属于本次验收。（存储/import 测试及 Controller 静态调用链）
...
- [x] AC13. PageIndexFallback 文件使用可识别的 private Workspace/extra marker，并按 Canvas.pageIndex 保存；当前进程再次进入稳定路径的同 key/source 且页数相同文稿时按相同页序号恢复，process-local 身份限 exact binding；其他进程或稳定 SlideID reader 不会把页号误认为真实 SlideID。（fallback 重入/升级/process-local 拒绝测试）

## Out of Scope

- 恢复或重新开放发布版白板入口及白板 UI 完善。
- 应用重启后的跨进程自动恢复、页面内容合并、插入/删除/重排后的绑定冲突解决及用户提示 UI。
...
- 本任务属于复杂跨层任务；技术设计和执行顺序分别见 `design.md`、`implement.md`。用户已批准按该基线实施。
