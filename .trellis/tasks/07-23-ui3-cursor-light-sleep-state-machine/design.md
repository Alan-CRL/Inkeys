...
- Raw Input 高频路径只把客户区鼠标点与至多三个矩形计算点到矩形的欧氏距离平方。
- 阈值使用 `(50 × zoom)²`，不执行平方根。进入可接受消息区域时无条件视为近距离。
- 50px 只控制第三光源目标可见性和是否因坐标变化唤醒渲染；5 秒期限始终从离开可接受消息区域开始。

## Rendering

- `borderCursorLightTargetVisible` 表示跟踪状态和 50px 判定共同得到的视觉目标。
- `BarUIRendering::PrepareFrameLighting` 将现有单向淡入改为可逆强度动画：保存当前强度、目标强度和 elapsed，用相同 300ms smoothstep 在 0 与 1 之间连续过渡。
- 目标不可见时保留最后光源点直至淡出完成；Raw Input 坐标在 50px 外不增加序号、不唤醒渲染。
