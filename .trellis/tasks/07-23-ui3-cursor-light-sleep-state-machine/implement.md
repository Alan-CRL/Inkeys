...
- [x] 重写第三光源输入更新：区域状态转换、固定截止时间、独立 `50 × zoom` 径向光圈和无关位置更新裁剪。
...
- [x] 静态检查第三光源使用 `50 × zoom` 独立半径，第一光源继续使用 480px。
...
> 2026-07-23 根据 60 FPS 测试视频复核，发现“到任意可见 UI 的最近距离”作为全局亮度乘数会让主栏替颜色栏托底，造成等距离位置的第三光源贡献不同。已改为第一光源 480px、第三光源 `50 × zoom` 的独立径向半径；可见区域距离只保留为渲染唤醒裁剪。完整 `Debug | ARM64` 构建为 0 error。
