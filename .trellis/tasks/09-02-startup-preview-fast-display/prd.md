...
- **PR-03 T0**：T0 必须在 SuperTop 分支结束后立即由 `steady_clock` 记录。5 秒阈值只从 T0 计算，不得从日志、RenderPipeline、Preview 首帧或 Bar 初始化开始计算。
...
- **PR-14 5 秒进度条**：T0+5 秒仍未完成时，约 180ms 渐显确定进度条；成功先在 120-160ms 内隐藏，失败不等 5 秒，立即绘制当前真实比例的红色帧。
