...
- More 面板复用 UI3 Surface/PointLight/关闭按钮和动画状态，标准单元 70 DIP、最多五列；显式组远端、强制组近端，分割线只在两组同时存在时绘制。主栏左右换边不反转逻辑顺序，上下方向只翻转物理行。
- More 交互覆盖外部点击继续分派、面板消费、X 悬停填充与按压缩放、拖出取消/抬起关闭、按钮动作前关闭及 `closeMoreAfterAction=false` 保持打开；打开 More 会关闭绘制提示、颜色选择器、粗细滑块和几何面板。三角 SVG 保持固定朝向，入口改由 Selected 状态和青色高亮表达展开。
