...
- 根面板、关闭按钮和分割线复用几何面板的 Surface/PointLight/动画风格；根面板关闭时以 More 按钮中心为 60×30 紧凑态，展开时沿上下方向使用同类 Back 关键帧。关闭按钮放在按钮网格右侧 35 DIP 窄栏，不额外增加顶部高度；保留 5 DIP 网格间距和主栏锚点净空，并限制到显示器工作区。
- More SVG 只旋转三角图形。打开和关闭分别沿相反方向累加 180 度；使用 EaseOutBack 曲线，图标尺寸不参与动画，快速反转从当前角度续接，静止后归一化整周角度。浮层网格保持 5 DIP 单元间距，主栏锚点保留独立 12 DIP 净空，并使用未截断的 Back 进度驱动面板几何以保留回弹。
