...
10. 本轮仅增强取证：来源查询结果、消息 event 关联、过滤前后状态、末触点依据和按键例外命中；记录既有 Bar Raw Input 数据及注册覆盖情况。保留当前已知缺陷以便对照复测。
11. 添加实际日志事件链检查器及自身测试，回放两份用户失败日志；再执行 ARM64 Debug Solution、无窗口测试与 diff/格式检查。硬件诊断结果等待用户反馈。

12. 用户批准按 origin 修复：将来源过滤集中到生产/测试共用入口，记录 system-touch-move 拒绝原因；加入已知日志时序与接管边界测试，执行完整 ARM64 Solution 和无窗口测试。
