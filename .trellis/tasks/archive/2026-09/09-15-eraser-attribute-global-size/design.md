...
- 所有工作区均保留四份5 DIP与四份16 DIP正常留白；极窄时只裁圆的可见/命中区域，不改圆、按钮或留白尺寸。
...
## 2026-09-16 精细视觉与命中设计
- 快速粗细圆的选中动画使用每项独立的可逆透明度进度；外环按圆心与真实半径绘制，半径包含固定视觉间隙，按压变换围绕圆心且不改变稳定态直径。
- 命中测试先裁剪到当前可见面板，再以点到圆心距离判断；矩形仅保留为dirty/bounds包络，不参与业务命中。
- 删除仅用于悬停内容提示的矩形绘制与续帧条件；灵敏度菜单、禁用齿轮提示等独立交互保持。
- 自动分割线复用主栏Divider高度/居中合同；箭头复用绘制属性笔类型扩展区的目标角度、缓动和图标表达，避免建立第二套开合语义。

## 2026-09-16 直拖释放换边修正
- 松手时以当前锚点中心差统一平移锁存的`main / anchor / work`，旧侧收拢到透明中点前继续使用重基准后的工作区，避免新工作区夹取造成首帧闪烁。
- 上下倒转总时长与绘制属性一致：父时间线前50%内加入时只补剩余收拢段并保留完整展开半段；超过50%时使用完整独立批次。两段统一消费全局动画倍速。
