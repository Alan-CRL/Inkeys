...
## 2026-09-16 精细视觉与交互调整
- 删除橡皮属性控件悬停时出现的内容提示矩形，不影响灵敏度菜单本身。
- 24/32/40 DIP快速粗细圆的选中态改为圆外留小间隙的Accent细圆环；选中和取消均渐显/渐隐，不再使用矩形或底部胶囊表达选中。
- 三个快速粗细圆提供与按钮一致的按下反馈，命中区域严格为圆形；圆外接矩形四角不响应点击。
- 自动粗细按钮内部70/20分割线与主栏分割线等高且垂直居中。
- 自动粗细右侧三角复用“笔类型”扩展入口的展开/收起图标及动画；保留70/20动作区、Down动作锁存与整体按压缩放。
