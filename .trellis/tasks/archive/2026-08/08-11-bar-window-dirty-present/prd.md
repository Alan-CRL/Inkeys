...
- Slider/颜色拖动等目标不可预知的连续交互使用控件预先定义的完整交互域；主按钮拖动直接移动现有 HWND，并在松手前保持布局方向和窗口尺寸不变，不因屏幕位置变化重建或逐帧改变资源容量。
...
- 除主按钮捕获期间的直接 HWND 位移外，动态窗口位置/尺寸只由渲染提交事务负责；直接 `SetWindowPos` 与 ULW 使用同一几何锁串行化。WindowService 继续负责创建、显隐、样式和 Z 序，但动态模式激活后不再另行 `SetBounds` 覆盖 Bar。
