...
正常呈现时，Bar 的几何提交由渲染线程内的 ULW 事务单独负责。主按钮捕获是唯一例外：拖动期间暂停 Bar 客户端续帧，输入线程使用 `SetWindowPos(..., SWP_NOSIZE)` 直接移动现有 HWND；它与 ULW 提交持有同一几何锁，松手或失败退出后清除直拖状态并重新请求一次完整呈现。WindowService 继续负责创建、显隐、样式和 Z 序；动态模式激活后不再对 Bar 发送独立 `SetBounds/SetWindowPos`。
