...
- 拖动静止保持锁定：交互循环改 `peekmessage` 轮询；当 `candidateChanged` 且位移不超过 `5×dpiZoom` 时累计静止时长。`0.5s` 置 `thicknessSliderHoldHintActive`，随后 `1.5s`（合计 `2.0s`）线性写 `thicknessSliderHoldProgress`，满后置 `thicknessSliderHoldLocked`。锁定期间禁止 `ApplyCandidateWidth`，圆点 `thicknessSliderThumbVisible=false`，轨道仍保持拉直。布局顺序为标注线徽标 → 文案 → 圆环（环直径=文字行高）。渲染侧用 `HoldHintOpacity` 管文字显隐、`HoldRingOpacity` 管圆环显隐（锁定后圆环淡出，不变白）、`HoldTextMix` 管灰→白文字。松手、捕获丢失、`CloseThicknessSlider` 统一清零三态。
...
## Future Notes（i18n / 其他笔型）

> 当前实现刻意保持中文布局假设；下列项在未来做 i18n 或扩展笔型时再落地，本期不改代码。

- **起点依赖标注线徽标**：hold-lock 文案/圆环的水平起点目前是 `annotationBadgeX + annotationBadgeW + gap`。无标注线入口的笔型（如激光笔）或标注线隐藏时，必须改用预览区左内容边距等不依赖该徽标的锚点。
- **i18n 宽度与右侧超限徽标**：完整“保持并固定粗细”在长语言下可能与右侧 `thicknessOverflow` 徽标重叠。届时需在布局阶段测量可用宽度；若放不下完整文案 + 圆环，则降级为最短文案（如“固定”）+ 圆环，并始终为右侧徽标预留安全区。
- **文案资源化**：未来短/长文案应进入 i18n，而不是继续硬编码中文字符串；降级策略应与语言包字段一起定义。
