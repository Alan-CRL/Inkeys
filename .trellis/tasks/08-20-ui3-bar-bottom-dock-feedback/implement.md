...
36. [x] 延迟居中布局补偿吸收，直到主栏批次时间线和 `x/w` 属性动画共同收敛；覆盖绘制进入选择的反向收缩尾帧。
37. [x] 定位上一版方向锁失效原因：方向缓存建立早于桌面初始放置、既存 `middleV` 未清除，且成功提交后直接改锚点跨越了快照事务。
38. [x] 拆分当前目标、最后成功稳定方向、居中锁存方向和显式换向批次；桌面首次分类前固定向右展开，白板保持原方向。
39. [x] 居中布局推进前从当前视觉值取消遗留换向批次，并局部以 `forceRestart + nullopt` 清除主栏、按钮、图标/文字与光源透明度中点；不修改全局动画同目标语义。
40. [x] 将居中补偿吸收改为“成功屏障排队 → 下一强制帧整窗重基准 → 失败完整回滚重试”的两阶段事务，并保持 pending/in-flight 到最终紧致 viewport 帧。
41. [x] 扩充 Headless 纯逻辑覆盖：桌面/白板初始方向、稳定方向成功推进、居中取消换向以及重基准排队/失败/重试/成功。
42. [x] 运行本轮完整质量门并静态复核命中快照、完整 ULW 替换、悬停恢复和单 target/单提交合同。
...
- [x] 完成选择模式反向收缩的方向锁修复后重新运行以上全部验证。
- [x] 完成换向关键帧取消与两阶段重基准事务后重新运行以上全部验证。

本轮最终验证已完成：ARM64-host `InkeysRepo.sln Debug|ARM64` 编译通过（0 error，6 条既有 `hashlib++` 转换警告），i18n check 通过，四份现有 Headless 二进制均以 `--no-window` 通过。独立质量审查额外纠正了首次方向初始化顺序和完整换向 `middleV` 检测；审查后已重新完整编译并复跑全部 Headless。未启动 GUI。
