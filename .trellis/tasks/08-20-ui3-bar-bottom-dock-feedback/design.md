...
成功呈现快照加入水平模式、阶段、主按钮刚性偏移、未形变主体中心、主栏实际端点、远端弹簧映射、本帧居中布局补偿和实际联合外框中心。tracker 只消费指针驱动的未形变主体中心；普通形态 barrier 仅确认窗口位移已提交，不得重设抓手或 tracker 基准，只有 DPI/显示环境变化可用未形变快照重基准。直接移动失败回滚和 ULW 失败重试都必须原子处理两轴 tuple。非拖动居中态先按当前插值布局计算实际联合中心，再把差值作为统一刚性平移叠加到水平映射；布局稳定后仅在完整呈现事务成功时把补偿吸收进主按钮稳定锚点，失败帧不推进。dirty、viewport、PointLight 逆映射、第三光源和命中必须区分主按钮刚性变换与主栏主体映射；容量包络同时预留横纵 24 DIP 形变及 Gaussian 外扩。
