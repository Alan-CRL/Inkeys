...
- 已知兼容边界：同一路径稳定文稿的增删/重排只在保存的 SlideID 集与当前集合仍有至少一个交集时自动恢复。若所有真实 SlideID 都换新，当前 `CompatibleSlideIdSet` 保守返回跨文稿冲突，结束页也暂不冷恢复；这是防止同路径被另一文稿覆盖后的跨文稿串墨迹，尚未提供可证明“全量替换仍为同一文稿”的额外身份。
