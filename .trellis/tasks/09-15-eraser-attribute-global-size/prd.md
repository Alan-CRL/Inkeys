...
## 2026-09-16 追加迭代
- 橡皮主面板、灵敏度菜单和提示浮窗统一位于主栏下层；清空继续锚定主栏橡皮入口，允许左右内容宽度不对称。
- 正常空间下圆组从面板边缘到分割线的四段留白均为16 DIP；自动整体90×70 DIP，body 70 DIP、arrow 20 DIP，按钮外侧及清空两侧标准留白均为5 DIP。
- 自动整体内部增加1 DIP发光分割线；背景、边框、内容、箭头和分割线共享整组按压缩放，开启态统一Accent光影。
- 新增`Drawing.Eraser.Automatic=true`总开关。关闭时所有普通入口和显式Speed策略均按Fixed解析，但不改五入口保存值；重新开启恢复原配置。活动接触锁存到Up，下一接触生效。
- 传统“绘制→橡皮擦”设置页增加同一总开关；五入口在关闭时仍可编辑保存。不修改Clear、Undo/Redo、尺寸/灵敏度曲线和输入队列。
