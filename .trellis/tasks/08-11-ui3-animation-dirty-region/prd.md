...
- R12：下方调试文字只显示实际帧率与无限制帧率；两者每帧采样/重绘但只在完整 1 秒统计桶结束时更新为上一桶平均值。实际帧率按成功呈现的墙钟时间计算；无限制帧率按同批帧排除 60 FPS pacing 等待后的工作时间计算。
...
- [ ] 下方文字只有实际/无限制帧率；一秒内数值锁存不变，下一秒显示上一完整统计桶的平均值，无限制值排除 pacing 等待。
