...
- Windows inertia 与 CPU fallback 的普通减速度统一为 `4000 DIP/s^2`，形成克制的网页级滑行；Pen hover/候选超时仍使用 `12000 DIP/s^2` 加强制动。
...
- 平移速度只由真实 Touch Move 或带实际位置变化的最终 Up 及其 QPC 更新，渲染空帧不得覆盖；最后 Up 前锁存 Windows 速度，并用 `releaseQpc - lastInputQpc` 判断最近 `100ms` 输入，不能使用绘制线程处理时刻。Windows inertia 的旧 Completed 必须重置，创建/启动/步进失败时由同参数 CPU 线性惯性接续。
