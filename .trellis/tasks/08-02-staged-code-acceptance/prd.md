...
- Stage 2.5 完成并提交前不执行提交审计；不把 HUD 实现与安全验收混入同一提交。
...
- [x] 热路径使用固定容量样本，普通绘制帧无新增堆分配、文件 I/O、GPU readback 或跨线程同步等待；文字和硬件采样最多每秒更新一次。
- [x] ARM64 Debug/Release 完整解决方案构建、两个配置的无窗口控制台测试与 `--drawing-perf` 通过，编码/换行和 `git diff --check` 通过。
- [ ] 因当前对话禁止 GUI，HUD 位置、透明度、字号、点击穿透和多 DPI 视觉效果明确保留为人工验证项。
