...
## Stage 2.5 Contact HUD Follow-up (2026-08-04)

- [x] HUD 文本改为中文性能统计/接触设备分类；每个活动 contact 一行显示设备、分派工具、RGBA 颜色、实时粗细、X/Y、压力、速度、接触面积、高度角和转动角。
- [x] 数值字段使用固定宽度格式，HUD 使用等宽字体；最小宽度提高到 1180 DIP（窄屏按工作区收缩），高度按主屏工作区动态增高。
- [x] contact 数据只在物理绘制帧复制和格式化，没有新增 timer、backbuffer 合成或 dirty rect；平均 FPS 与无等待性能 FPS 仍按 1 秒窗口汇总。
- [x] ARM64 Debug/Release 完整解决方案、两个配置全量无窗口测试和 Release `--drawing-perf` 通过；C++/测试文件保持 UTF-8 BOM + CRLF。
- [ ] 按当前对话限制未启动主程序或 HUD 窗口；中文字体回退、固定列视觉对齐、面板宽高与多 DPI 仍待人工验证。

## Stage 2.5 Per-Frame HUD Follow-up (2026-08-04)

- [x] 平均帧率和无等待性能 FPS 保持 1 秒统计窗更新；平均帧时、1% Low、P99、帧波动、绘制/呈现耗时、CPU、内存、显存和 contact 行改为每个物理绘制帧更新。
