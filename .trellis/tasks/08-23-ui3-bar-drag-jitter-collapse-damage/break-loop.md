...
- 此前的目的地策略假设 frame serial 始终代表它消费过的几何。渲染强制 Free/Dragging -> Stable 却可能把旧浮动帧升级到新捕获 serial，破坏了这个前提；原测试只构造合法 frame tuple，因此没覆盖虚假 barrier 确认后再直移旧位图的链路。
- 现在通过 CAS 写者独占、渲染只能认领已消费且未抓取的版本，以及保留 tracker 的拖动阶段，阻止旧帧借用新输入身份。回归明确串联旧帧、捕获、错误确认与下一次微小 X/Y 直移。
- 捕获事件以前按上一候选 mode 消费；跳帧/失败后可能丢失初值。现在以成功呈现模式和实际底边播入，捕获项精确保留，普通抓手限值不变。
- 作废发生在 AddDemand 之前时，保留 dirty 不等于保留呈现请求。RequireVisualRetry 将两者共同保留至成功，防止鼠标静止或动画刚结束后错过正确画面。
