...
- 96 DPI、基准压力下白芯与红色实体外套的总直径为 10px，白芯保持实体直径的三分之一；红色实体轮廓沿法线向外固定漫反射 3px，因此完整视觉直径为 16px。实体尺寸随 DPI 和 Pen 压力缩放，3px 漫反射只随 DPI 缩放并覆盖完整 dirty bounds。
- Pen 压力按 `0.65 + 0.75 * clamp(p, 0, 1)` 缩放实体总直径，Mouse/Touch 与 Hover 保持 10px 基准实体直径；无效压力沿用上一有效宽度，prediction 沿用最后真实宽度。
...
- [ ] 96 DPI 下实体总直径为 10px、白芯约 3.33px、两侧漫反射各 3px；Hover/Touch tip 与固定宽度轨迹一致，Pen 压力只缩放实体。
