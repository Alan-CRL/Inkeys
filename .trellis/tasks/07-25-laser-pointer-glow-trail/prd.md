...
- 粒子由 D3D11 Compute Shader 在固定 2048 槽结构化缓冲区中模拟；Down 只建立追加式真实路径，不生成起笔爆发，移动时按每 2px 一枚发射且全局每帧最多 96 枚。
- 粒子保存路径 slot/generation、真实弧长和 segment cursor，沿已确认真实曲线转弯；prediction 不上传，整笔逐帧重绘与追加真实点都不得重建或跳变既有粒子。
- 粒子出生于白芯，约 120ms 内穿过红色实体外套并发散到红边外最多 1.5 DIP；粒子始终贴近整条真实路径，解析式亮核外带约 3 DIP 辉光。
- 活动笔画的存活粒子以 80ms 时间常数持续平滑追随最新滤波画笔速度；寿命 0.45-0.70s、最大沿线行程 24-56 DIP，末段平滑淡出。到达当前真实路径末端时钳制且不积累待追赶距离。
- Up 停止生成和沿弧长推进，75% 收束至当前弧长处红边、25% 收束至中心线，在约 220ms 内缩小并淡出。
...
- [ ] GPU 粒子默认开启且数量受限；Down 无爆发，慢/快画时平滑追速，急弯与追加真实点时沿曲线连续运动，Up 按 75/25 收束，关闭能清除旧粒子且不改变主轨迹。
