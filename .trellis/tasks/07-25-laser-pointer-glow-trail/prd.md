...
- 粒子出生锚点跟随 L0 最前端，异常跳变以 120ms 时间常数靠近；出生后约 120ms 穿过红色实体外套并发散到红边外最多 4.5 DIP。白色亮核具有每粒独立的明暗与柔和呼吸，3 DIP 外辉光为浅粉红而非主体深红。
...
- [ ] GPU 粒子默认开启且数量受限；Down 无爆发，静止仍按时间生成，移动密度约每 4 DIP 一枚；慢/快画、急弯、L0 尾替换与预测回缩时连续且不补发，Up 后继续减速淡出至固定 3 秒寿命。
- [ ] 粒子具有独立基础亮度和异步呼吸，白芯外为可在白底辨认的浅粉红 3 DIP 辉光，法线发散可到实体红边外 4.5 DIP；呼吸不改变 Alpha。
