...
- 96 DPI、基准压力下白芯直径约 5px、红色实体外径约 15px、完整红晕外径约 28px、白红散射带约 1px，全部随 DPI 和 Pen 压力同比缩放并覆盖完整 dirty bounds；材质顺序为柔光、红边、约 1px 红粉外缘高亮、内侧散射、纯白芯。
- Pen 压力按 `0.65 + 0.75 * clamp(p, 0, 1)` 缩放整组材质，Mouse/Touch 与 Hover 保持基准尺寸；无效压力沿用上一有效宽度，prediction 沿用最后真实宽度。
