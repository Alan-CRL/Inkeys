...
- 粒子出生锚点跟随 L0 最前端，异常跳变与预测回缩保持限速修正状态，并按约 2 倍正常粒子速度追赶至目标；出生后约 120ms 穿过红色实体外套并发散到红边外最多 10 DIP。白色亮核具有每粒独立的明暗与柔和呼吸，3 DIP 外辉光为浅粉红而非主体深红。
- 每粒固定寿命 3.0s；速度倍率和 Alpha 都使用 `1 - smoothstep(0, 1, age / 3)` 非线性降至零，不再按行程结束。活动期间基础流速仍以 80ms 时间常数追随最新滤波画笔速度；到当前组合路径末端时钳制且不积累待追赶距离。
- Up 只停止发射与路径追加，冻结最后输入速度；存量粒子继续沿最终路径减速、淡出至自身 3 秒寿命结束，不再执行 75/25 收束。当前批次实际安排过粒子时，有效 Hold 至少为 3.0s，但公开设置值与 getter 保持不变。
...
- [ ] GPU 粒子默认开启且数量受限；Down 无爆发，静止仍按时间生成，移动密度约每 4 DIP 一枚；慢/快画、急弯、L0 尾替换与预测回缩时连续且不补发，异常修正约按 2 倍正常粒子速度限速追赶，Up 后继续减速淡出至固定 3 秒寿命。
