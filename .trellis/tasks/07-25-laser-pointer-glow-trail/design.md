...
`EmitCS` 在当前组合路径末端与 CPU 平滑出生锚点创建粒子，并写入 CPU 管理的循环粒子槽；`UpdateCS` 保存并推进每粒子的组合弧长、segment cursor、平滑位置/切线、横向偏移、速度、3 秒寿命和呼吸状态。路径采样优先使用 cursor，跨越过多段时用固定次数二分查找；预测尾缩短时弧长立即钳制，但持久位置/切线正常使用 40ms 响应，异常跳变与回缩则保持修正状态并按当前生命周期实际沿线速度的 2 倍限速，直至真正到达新目标，丢弃的位移不追赶。出生锚点正常跟随 L0 前端，超过 `max(6*dpiScale, 2*filteredInputSpeed*dt + 2*dpiScale)` 的跳变同样保持修正状态，并按目标粒子速度的 2 倍限速追赶至目标。
