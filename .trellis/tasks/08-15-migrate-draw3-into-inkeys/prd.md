...
- 保持现有窗口 owner 链和 UI/PPT owned-popup 关系，Draw3 只作为 Drawpad 窗口链中的绘制实现。
...
- 迁移必须保持透明像素、dirty rect、点击穿透、窗口层级和 owned-popup 行为正确，ULW 不得以整窗不透明内容遮挡 Drawpad 下层窗口。
...
- 已完成：`Build\\ARM64\\Debug\\Inkeys.exe --draw3-hidden-test`，退出码 0；测试顺序创建 DComp-compatible 与 legacy-compatible 隐藏 Drawpad HWND，真实验证旧 DComp HWND 上 ULW 启动失败及 Host 清理，再重建唯一 legacy 窗口链并运行 DComp/DWM2/DWM/ULW presenter，覆盖命令、resize、首帧、样式真值和停止流程，未显示产品窗口。
