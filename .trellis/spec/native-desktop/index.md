...
- Draw3 由唯一 Host/RTS 绑定 Window Service 已创建的主 `WindowRole::Drawpad` HWND；同一 Host 可把 `WindowRole::DrawpadPresentation` 前置表面作为 selection-only ULW 呈现目标，但辅助窗不绑定输入、document 或第二套 runtime。Draw3 的 D3D11.1 device、swap chain 和 DComp/DWM/ULW presenter 均独立于 `Inkeys.UI.RenderPipeline`，资源 ID 301-304 由 `draw3-integration.md` 统一维护。
