...
- `BarToggleClickCoalescer::TryBegin(channel, now)` 对每个 `BarToggleChannel` 独立记录最近一次成功 toggle；第一击立即通过，距上次同通道操作小于 180ms 时返回 `false`，达到 180ms 边界后重新通过。
...
- Headless 测试必须覆盖首次立即执行、179ms 合并、180ms 边界通过和不同通道互不阻塞；按钮调用点另以静态审查和完整 Solution 构建确认门控位于状态切换之后。
