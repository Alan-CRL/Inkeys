...
收起终点和换边中点的根面板均以锚点为中心，并限制为固定紧凑尺寸。直属 Shape 的隐藏宽高、圆角和边框使用固定展开到紧凑倍率，自身中心限制在紧凑面板中心；嵌套 SVG/Word 的局部 `x/y` 归零并等比缩小。隐藏中点不得使用运行中的子对象 `val` 乘“紧凑宽度 / 当前面板宽度”，因为位置动画被打断或重启时，两者不再保持同一比例，会放大旧的展开坐标。
