...
- 普通底栏提示资格在手势开始时从成功呈现模式锁存，只有从 `Floating` 开始的展开主栏手势进入底栏后可显示；底栏起始手势的居中提示资格在本手势真实发生 `Free → Centered` 时建立，并在有效底栏拖动中的 `Centered → Free → Centered` 全程保持。Free 阶段显示“底栏模式”，Centered 阶段显示“底栏模式 · 居中”；只有松手、折叠、取消或竖向脱离时清除。文案变宽必须先完成外框扩宽再交换文字，变窄必须先完成文字交换再缩窄外框；命中仍只发布成功呈现的实际外框。
