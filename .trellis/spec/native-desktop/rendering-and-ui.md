...
- 主按钮与主栏使用同一个二维主体映射。水平映射以主按钮中心为精确跟手支点并吸附另一远端：右向展开从中心左侧进入时拉伸、越过中心后压缩，左向展开镜像；捕获远端和脱离恢复必须从上一成功像素连续播入。绘制、dirty、viewport/capacity、PointLight 逆映射、第三光源接受区和成功命中边界必须从同一映射解析；两轴 24 DIP 视觉限值、Gaussian 外扩和抗锯齿余量都进入保守包络。
- 启动时已有的展开中置底栏发布为稳定居中。折叠时退出居中但不移动主按钮；底栏重新展开时只在最终展开联合外框仍位于阈值内时无提示捕获。
- 普通底栏提示资格在手势开始时从成功呈现模式锁存，只有从 `Floating` 开始的展开主栏手势进入底栏后可显示；居中提示资格则在本手势真实发生 `Free → Centered` 时建立，因此从 `BottomDocked` 开始也可显示“底栏模式 · 居中”，离开居中带、松手、折叠或竖向脱离时清除。文案变宽必须先完成外框扩宽再交换文字，变窄必须先完成文字交换再缩窄外框；命中仍只发布成功呈现的实际外框。
...
| 底栏起始手势再次拖入中心 | 可进入居中并显示居中提示；离开居中带后淡出 |
...
- Good：Draw3 绘制线程只向已请求且就绪的 target present；双窗尺寸与互斥显隐通过 Window Service；Bar 与五个 PPT 是同 owner 的兄弟窗口，PPT 前置始终止于 Bar 正下方。
