...
- 启动时已有的展开中置底栏发布为稳定居中。折叠时退出居中但不移动主按钮；底栏重新展开时只在最终展开联合外框仍位于 40 DIP 阈值内时无提示捕获。非拖动的 `BottomDocked + Centered + Expanded` 每帧都以当前插值布局的实际联合外框求中心补偿，并把补偿作为水平映射统一刚性平移；布局稳定后只能在完整呈现成功时吸收到稳定锚点，失败帧不得推进或累积。
...
- Headless 覆盖二维中心不变量、动画中布局补偿、扩展面板锚点、首次显现完整光影包络、失败快照，以及浮动进入/底栏双向切换/未切换手势的提示资格。
