...
荧光笔不使用实时笔锋，时间保护窗口只包含预测时长；此外还会在 L0 中保留末端至少 12px 的真实路径，专门用于稳定平直尾帽方向和跨层圆角。
...
荧光笔不再为每个点保存独立走势，也不再把相邻横截面直接拼成方向变化四边形。连续点距离不超过 `0.01px` 时作为重复点移除；每个有效中心线段生成固定宽 50px 的解析矩形 body。只有内部端点沿切线各重叠 2px，用于覆盖独立 primitive 的抗锯齿端边；全局起终点不延伸，保持严格止于输入点的 butt cap。

相邻线段转角大于 `0.5°` 时，在外侧补半径 25px 的解析圆弧 sector，径向边不单独做抗锯齿，由相邻 body 重叠覆盖；转角达到 `177°` 时改用完整 50px 圆片处理近似折返。矩形外轮廓、平直端帽和圆弧外轮廓仍使用 `fwidth + smoothstep`。CPU 脏矩形与 GPU AABB 使用相同的 body 四角和 join 半径，并额外保留 3px 安全余量。

长笔画的起始方向取首段真实路径 12px 的弦方向，达到 12px 后锁定；锁定前起笔不提交 L1。可见末端方向取真实点与预测点最后 12px 的弦方向。构造 primitive 时把首尾 12px 内的密集点折叠为稳定弦段，中间点保持不变；L0 还会保留提交边界前一个上下文点和末端至少 12px 的真实路径，使跨 L1/L0 的 round join 完整。

短划只按真实建模路径长度分类，预测长度不参与判断。真实路径不足 12px 时不显示预测：纯点击按住期间不显示，有实际移动时只实时显示实际短路径。抬笔后丢弃预测后缀，并从鼠标按下点沿“按下点到真实终点”的弦方向生成确定性的 `12×50px` 矩形；若终点回到起点则使用第一个有效移动方向，完全没有移动时使用 `+X`。因此纯点击得到竖直长方形，点击点位于其左侧中点，而不是居中的方形印记。
